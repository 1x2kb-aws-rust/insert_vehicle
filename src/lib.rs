...
mod base_64_should {
    ...
    use crate::base_64_decode;
    const DECODED_UTF: [u8; 81] = [
        123, 34, 109, 97, 107, 101, 34, 58, 32, 34, 67, 104, 101, 118, 114, 111, 108, 101, 116,
        34, 44, 32, 34, 109, 111, 100, 101, 108, 34, 58, 32, 34, 83, 105, 108, 118, 101, 114,
        97, 100, 111, 34, 44, 32, 34, 109, 111, 100, 101, 108, 95, 121, 101, 97, 114, 34, 58,
        32, 34, 50, 48, 50, 50, 34, 44, 32, 118, 105, 110, 58, 32, 34, 48, 49, 50, 51, 52, 54,
        55, 34, 125,
    ];
    ...
    fn decode_valid_string_with_matching_string() {
        ...
        let result = base_64_decode(encoded).map(String::from_utf8).unwrap().unwrap();
    ...
    fn not_decode_invalid_string() {
        ...
        assert!(result.is_err(), "base_64_decode parsed correctly but should have been an error");
    }
}
